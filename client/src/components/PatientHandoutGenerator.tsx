...
<body>
  ...
  const generateHandout = () => {
    ...
    if (!html) return;
    // Create a Blob URL and trigger download as HTML file
    const blob = new Blob([html], { type: 'text/html' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${currentTemplate?.name?.replace(/\s+/g, '-') || 'patient-handout'}.html`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    // Delay revoking to allow browser to complete the download
  ...
  const previewHandout = () => {
    const html = buildHandoutHTML();
    if (!html) return;
    // Use an iframe approach that doesn't trigger popup blockers
    const blob = new Blob([html], { type: 'text/html' });
    const url = URL.createObjectURL(blob);
